...
2. Create a new Render Blueprint from the `backend/` directory.
3. Set `GOOGLE_API_KEY` in Render environment variables.
4. After deploying the frontend, set `ALLOWED_ORIGINS` to the frontend URL.
5. Deploy the `concierge-support-adk` web service.
6. Verify `https://your-service.onrender.com/healthz`.

The Render start command ingests the bundled `docs/` corpus into Chroma before
starting Uvicorn:
...
```bash
docker build -t concierge-support-adk .
docker run --env GOOGLE_API_KEY=your-google-api-key -p 8000:8000 concierge-support-adk
```

Use a stronger persistent database and vector store before treating this as a
production system. The default live config keeps the assignment self-contained
with SQLite and local Chroma storage.
