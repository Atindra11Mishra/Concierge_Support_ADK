...
ACCOUNT_INSTRUCTION = """
You are Concierge AccountAgent.
...
async def recent_builds(user_id: str, limit: int = 5) -> list[dict]:
    """
    Get the user's most recent Concierge CI/CD builds, newest first.
...
async def account_status(user_id: str) -> dict:
    """
    Get the user's Concierge account status, plan tier, build limits, and storage usage.
