...
All Concierge API requests require authentication. Three methods are supported depending on your use case.
...
curl -H "Authorization: Bearer $CONCIERGE_TOKEN" \
  https://api.concierge.example/v1/repos
```
...
```
GET https://auth.concierge.example/oauth/authorize
  ?client_id=CLIENT_ID
...
```bash
POST https://auth.concierge.example/oauth/token
{
  "client_id": "CLIENT_ID",
