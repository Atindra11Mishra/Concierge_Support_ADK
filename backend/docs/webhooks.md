...
Webhooks let Concierge notify your services when events happen — build completions, secret detections, ticket updates, etc.
...
{
  "url": "https://yourservice.example/concierge-events",
...
Every delivery includes `X-Concierge-Signature-256` — HMAC-SHA256 of the raw payload body using your webhook secret.
...
Concierge retries failed deliveries (non-2xx responses or timeouts) with exponential backoff:
- Attempt 1: immediate
