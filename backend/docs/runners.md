...
Runners are the compute environments that execute your pipeline steps. Concierge provides cloud-hosted runners and supports self-hosted runners for custom requirements.
...
```bash
# Linux x86_64
curl -sSL https://dl.concierge.example/runner/install.sh | bash

# Configure
concierge-runner configure \
  --url https://concierge.example \
  --token $RUNNER_REGISTRATION_TOKEN \
  --name my-runner-01 \
  --labels linux,docker,fast-storage

# Start as a service
sudo systemctl enable --now concierge-runner
```
...
Use the Concierge Runner Autoscaler to scale based on queue depth:
...
- Self-hosted runners should run with a dedicated non-root user: `concierge-runner`.
- Network egress: runners need outbound HTTPS to `api.concierge.example` and your artifact registries.
- For highly sensitive builds, use `runner.network: isolated` to disable all outbound except Concierge.
...
| `runner disconnected` error | Network interruption | Check runner logs: `journalctl -u concierge-runner -n 100` |
