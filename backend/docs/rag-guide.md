...
table = db.create_table("concierge_docs", schema=schema, mode="overwrite")
...
```python
faiss.write_index(index, "concierge.index")
index = faiss.read_index("concierge.index")
```
...
```
You are a Concierge product knowledge agent.
