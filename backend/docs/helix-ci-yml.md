---
title: .concierge-ci.yml Reference
product_area: ci-cd
tags: [configuration, yaml, pipeline, steps, conditions]
---

# .concierge-ci.yml Reference

Your pipeline is defined in `.concierge-ci.yml` at the repo root.
...
pipelines:
  build-and-deploy:
    ...
    steps:
      - name: checkout
        uses: concierge/checkout@v2

      - name: setup-python
        uses: concierge/setup-python@v1
        with:
      ...
      - name: push-image
        run: docker push registry.concierge.example/myapp:${{ git.sha }}
        condition: ${{ git.branch == 'main' }}
        env:
          REGISTRY_TOKEN: ${{ secrets.REGISTRY_TOKEN }}

      - name: deploy
        run: concierge deploy --env production --image myapp:${{ git.sha }}
        condition: ${{ git.branch == 'main' }}
