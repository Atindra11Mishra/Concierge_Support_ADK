...
**Via push:** Any push to a tracked branch triggers the pipeline defined in `.concierge-ci.yml`.

**Manual trigger via API:**
```bash
curl -X POST https://api.concierge.example/v1/repos/{repo_id}/builds \
...
Build artifacts (binaries, coverage reports, container images) are stored in Concierge Artifact Registry.

```bash
# Download artifact
curl -L https://artifacts.concierge.example/{build_id}/{artifact_name} \
  -H "Authorization: Bearer $CONCIERGE_TOKEN" -o artifact.tar.gz
```
...
4. Reproduce locally: `concierge run --local --pipeline test --ref HEAD`.
...
| `No such file or directory: .concierge-ci.yml` | Missing config file in repo root |
