...
ssh-keygen -t ed25519 -f ~/.ssh/concierge_deploy_new -C "ci-rotated-$(date +%Y%m%d)"

# Step 2: Add the new public key in the Concierge UI (Settings → Deploy Keys → New)
# Keep the old key active during this step.

# Step 3: Update your CI/CD secret store with the new private key.
#   GitHub Actions: Settings → Secrets → CONCIERGE_DEPLOY_KEY
#   GitLab CI:      Settings → CI/CD → Variables → CONCIERGE_DEPLOY_KEY

# Step 4: Trigger a test pipeline run to confirm the new key works.

# Step 5: Delete the old key from Concierge UI.
```
