...
Concierge Artifact Registry stores build outputs: Docker images, Python wheels, npm packages, and generic binaries.
...
concierge registry login

# Or manually
echo $CONCIERGE_TOKEN | docker login registry.concierge.example \
...
```bash
docker build -t registry.concierge.example/{org}/{image}:{tag} .
docker push registry.concierge.example/{org}/{image}:{tag}
```

In CI:
```yaml
- name: push-image
  uses: concierge/docker-push@v1
  with:
    image: registry.concierge.example/myorg/myapp
    tag: ${{ git.sha }}
```
...
```bash
docker pull registry.concierge.example/{org}/{image}:{tag}
```
...
# Publish
python -m build
twine upload --repository concierge dist/*

# Configure in pyproject.toml
[[tool.poetry.source]]
name = "concierge"
url = "https://pypi.concierge.example/{org}/simple/"
```
...
```bash
# .npmrc
registry=https://npm.concierge.example/{org}/
